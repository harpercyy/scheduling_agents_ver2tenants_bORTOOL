...
系統採用多租戶（Multi-Tenant）架構，每個店面（租戶）為一個獨立的 `tenants/<tenant>/` 目錄。所有店面特定的設定（班次定義、角色對應、人力需求、假日表）均透過 `tenant_config.json` 統一管理，腳本本身不 hard-code 任何租戶邏輯。
...
> **詳細 schema 與操作說明**：請參考專案根目錄的 `CLAUDE.md`。
