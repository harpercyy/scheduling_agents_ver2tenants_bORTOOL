...
## 6. availability.json + rest_days.json 合併載入

系統會**同時載入**兩個檔案，取 `designated_rest` 的聯集：
- `availability.json` → FT 公休日 + PT 可用時段
- `rest_days.json` → FT 指定休假（跨週持久）

兩者的 `designated_rest` 會合併（相同日期自動去重）。建議用法：
- **`rest_days.json`**：放 FT 的長期指定休假（不隨週變動）
- **`availability.json`**：放每週 PT 可用時段 + 公休日（全員休假）

> **注意**：不再是 fallback 關係。即使 `availability.json` 存在，`rest_days.json` 的休假仍會被載入。
